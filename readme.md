...
- `models/Level.js` – define objetivo de pontuação, velocidade da nave e clusters; controla o deslocamento do background na troca de nivel.
...
- `Asteroid` (herda de `Entity`): obstáculo que gira, avança em direcao ao player e pode ser respawnado (`canRespawn`).
...
- Esse jogo provavelmente não é o melhor na questao de performance (o mais dificil é manter os asteroides do nivel anterior vivos ate eles terem saido da tela para garantir que nao vao sumir na frente do usuario. Por causa disso eu processo todos os niveis anterioes com `processLevel()`), mas acredito eu que não haja nenhum problema no sentido de encher a memmória do computador se jogar por muito tempo.